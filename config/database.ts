...
export default {
  ...
  models: {
     ...
     */
    // ERBA defines Store, Special and its own Product in `app/Models`, but the
    // storefront runs on the built-in commerce models (Category, Manufacturer,
    // Cart, Order, Coupon, ...) without publishing each one. Without this,
    // adding the first app model stops those 68 tables being generated and the
    // next `generate:migrations` emits drops for them.
